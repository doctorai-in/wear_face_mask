...
class FaceMasker:
    ...
    def process_roi(self, rois):
        ...
        #confidence = []
        for roi in rois:
            xmin, ymin = tuple(roi.position)
            xmax, ymax = tuple(roi.size + roi.position)
            box.append((int(ymin), int(xmax), int(ymax),int(xmin) ))
            #confidence.append(roi.confidence)
        return box
    def mask(self):
        import face_recognition

        face_image_np = face_recognition.load_image_file(self.face_path)
        #frame = cv2.imread(self.face_path)
        #face_locations = face_recognition.face_locations(face_image_np, model=self.model)
        #print(face_locations, type(face_locations))
        rois = frameprocessor.detect_faces(face_image_np)
        #print(rois)
        face_locations = self.process_roi(rois)
        #print(face_locations, type(face_locations))
...
if __name__ == '__main__':
    dataset_path = '/home/ankit/wear_mask_to_face/vggface2/vggface2_test/test'
    save_dataset_path = '/home/ankit/wear_mask_to_face/vggface2/test-with-face-mask'
    start=0
    for root, dirs, files in os.walk(dataset_path, topdown=False):
        folder = root.rsplit("/")[-1]
        print(folder)
        if folder == "n000689" and  start == 0:
            start = 1
            for name in files:
                try:
